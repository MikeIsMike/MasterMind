	...
	void create_attempt(std::vector<int>& attempt) {
		...
		if (potentialSet.size() == 1) {
			...
			for (int i = 0; i < fullSet.size(); i++) {
				int max = 0;
				for (int j = 0; j < hitSet.size(); j++) {
					int score = 0;
					for (int k = 0; k < potentialSet.size(); k++) {
						int black_hits_1 = 0, white_hits_1 = 0;
						tmpMaker.change_sequence(potentialSet[k]);
						tmpMaker.give_feedback(fullSet[i], black_hits_1, white_hits_1);
						if ((hitSet[j][0] == black_hits_1) && (hitSet[j][1] == white_hits_1)) {
							score++;
						}
					}
					if (score > max) {
