...
//		}
	...
	void create_attempt_bigSet(std::vector<int>& attempt) {
		if (!triedEveryNum) {
			...
			attempt = prevAttempt;
			attempt[numIndex1] = possibleNums[possibleIndex1];
			attempt[numIndex2] = possibleNums[possibleIndex2];
			if (correctNum) {
				numIndex1++;
				possibleNums.erase(possibleNums.begin() + possibleIndex1);
				possibleIndex1 = -1;
				prevAttempt = attempt;
				correctNum = false;
			}
				...
				numIndex2++;
				possibleNums.erase(possibleNums.begin() + possibleIndex2);
				possibleIndex2 = -1;
				prevAttempt = attempt;
				correctNum2 = false;
			}
	...
	void learn_bigSet(std::vector<int>& attempt, int black_hits, int white_hits){
		if (!triedEveryNum) {
			...
			if (prev_black_hits == black_hits+1) {
				correctNum2 = true;
				possibleIndex2--;
				possibleIndex1++;
			}
...
//			solver.learn(attem, black_hits, white_hits);
			...
			solver.create_attempt_bigSet(attem);
			maker.give_feedback(attem, black_hits, white_hits);

			solver.learn_bigSet(attem, black_hits, white_hits);
			number++;
			att = attem;
		}
	...
	std::cout << sum / pow(num, length) << std::endl;
	std::cout << totalduration / pow(num, length) << std::endl;
